...
use bangladesh::shared::world::{
    TerrainKind, TerrainTile, WorldMetadata, world_output_path, write_world_file,
};
use clap::Parser;
...
fn downsample_parent_tile(
    ...
    let mut parent_cells = vec![TerrainKind::Unknown.code(); cells_per_side * cells_per_side];
    ...
    for parent_y in 0..cells_per_side {
        for parent_x in 0..cells_per_side {
            ...
            let mut resolved = TerrainKind::Unknown.code();
            for sample_y in 0..2 {
                for sample_x in 0..2 {
                    let source_index = (child_base_y + sample_y) * cells_per_side
                        + (child_base_x + sample_x);
                    resolved = merge_terrain_code_with_priority(resolved, child_cells[source_index]);
                }
            }
